...
export class Player{
    ...
    checkCollision(){
        function playAudio() {
            let audio = new Audio("Assets/pop-5.mp3");
            audio.play()
        }
        this.game.enemies.forEach(enemy => {
            if (enemy.x < this.x  + this.width && enemy.x + enemy.width > this.x && enemy.y < this.y + this.height && enemy.y + enemy.height > this.y) {
                if (this.rollingCheck == true) {
                    this.game.score++;
                    this.game.particleX = enemy.x;
                    this.game.particleY = enemy.y;
                    this.game.sas = true;
        ...
        this.game.platforms.forEach(platform => {
            if(platform.x < this.x  + this.width && platform.x + platform.width > this.x && platform.y < this.y + this.height && platform.y + platform.height > this.y){
                this.game.groundMargin = platform.x;
            }
        });
    }
}